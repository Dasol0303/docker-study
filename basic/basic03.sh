...
# 도커 컨테이버는 어떠한 실행되는 프로세스가 존재해야 컨테이너가 계속 실행상태로 유지됨
# 프로세스가 없다면 컨테이너는 바로 종료됨

# busybox 이미지로 무한 sleep 상태로 실행
docker run -d --name busybox busybox sleep infinite
# 테스트용 작은 리눅스 
# sleep 무한으로 돌아라! 라는 프로세스가 수행 중이기 때문에 컨지이너가 종료되지 않고 유지
...
# -it 실시간 대화형 (현재 터미널 상에서 연결된 상태로 사용)
