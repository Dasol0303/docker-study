...
# 도커 파일을 기반으로 새로운 이미지를 "빌드"


# 도커파일 기반 빌드

# 현재 경로에 있는 Dockerfile에 작성된 내용을 기준으로 새로운 이미지를 빌드하겠다(만들곘다)
docker bulid -t 이미지이름 .
...
# Dockerfile 에 ENV 설정이 되어있어서, 별도로 -e PW 세팅을 안하고 실해해도 된다
docker ps -a
# 비밀번호같은 중요한 정보는 ENV로 설정을 고정해두는 것보다, 실행 시점에 -e 옵션으로 지정하는 것을 권장
